...
- **O(1)**: 입력 크기 N과 무관하게 거의 일정
    ...
    - 🚨 위험 구간: 없음. 데이터가 수억 개든 수천억 개든 메모리(RAM)만 버텨주면 연산 시간은 무조건 통과.
- **O(N)**: N이 2배면 시간도 대충 2배
    ...
    - 🚨 위험 구간: $N \approx 10,000,000 ~ (10^7)$ (1천만). 파이썬 순수 for문 기준 1천만 개부터 슬슬 1초를 넘기기 시작함. 1억 단위로 넘어가면 C 레벨로 구현된 numpy나 data.table 같은 벡터화 연산이 강제됨. (WF 10,000매 정도네)
- **O(N^2)**: N이 2배면 시간은 대충 4배
    - $Cost(N) \approx N \times N$
    - 🚨 위험 구간: $N \approx 10,000 ~ (10^4)$ (1만). $N$이 1만만 되어도 연산량이 1억 번($10^8$)이라 1초 컷에 걸릴 위험이 큼. (WF 10매 정도네)

> N이 10만을 넘어가는 순간부터는 $O(N^2)$은 쳐다도 보면 안 되고, 최소 $O(N \log N)$ (정렬 알고리즘 등)이나 $O(N)$ (해시 테이블, 투 포인터 등)으로 로직을 우회해야 한다
...
### 1. 🐢 발단: "파이썬 `for` 문은 왜 느려 터졌나?" (Overhead)

* **현상:** 같은 $O(N)$인데 파이썬은 느리고, C/NumPy는 빠르다.
* **원인:**
  * **Python:** 루프 한 번 돌 때마다 "타입 검사 + 메서드 찾기 + 참조 카운팅"이라는 **비싼 결재 서류(Overhead)**를 쓴다.
  * **NumPy/R:** "결재"는 처음에 딱 한 번 하고(O(1)), 내부는 **C언어(기계어)**가 폭주 기관차처럼 달린다.


* **교훈:** **"Loop를 내가 돌리지 마라. C로 짜인 용병(Vectorization)에게 맡겨라."**

### 2. 💾 전개: "범인은 메모리(RAM)에 있다" (Memory Hierarchy)

* **스택(Stack) vs 힙(Heap):**
  * **Stack:** 데이터가 차곡차곡(Sequential). **DRAM Row Hit**가 터져서 미친 듯이 빠르다. (C언어 지역변수)
  * **Heap:** 데이터가 난장판(Random). **DRAM Row Miss**가 터져서 문 열고 닫느라 시간 다 간다. (파이썬 객체)


* **물리적 실체 (DRAM Cell):**
  * 파이썬 리스트(`list`)가 느린 건, **흩어진 주소를 찾아다니느라(Pointer Chasing)** DRAM의 **Word Line**을 계속 껐다 켰다 하기 때문이다.
  * 반면 **NumPy**는 데이터가 붙어 있어서(Contiguous), 한 번 열었을 때 **뽕을 뽑는다(Burst Mode).**



### 3. 🚀 절정: "찾지 말고 계산해라" (Hash Table)

* **문제:** 리스트에서 데이터 찾으려면 방명록을 다 뒤져야 함 (O(N)).
* **해결:** **Key가 곧 주소(Address)다.**
  * "찾기(Search)"를 **"수학 계산(Hash Function)"**으로 대체함.
  * `Hash("Key") -> 메모리 582번지` (계산 끝, 바로 이동).

* **결과:** 데이터가 100만 개든 10억 개든, **단 한 번(O(1))의 접근**으로 데이터를 낚아챈다. (RAM을 가장 덜 괴롭히는 기술)

### 4. ⚔️ 결말: "공간을 팔아 시간을 산다" (Parallel Processing)

* **개념:** 혼자 하기 벅차니 분신술(`%dopar%`, `joblib`)을 쓴다.
* **대가:**
  * **RAM 복사:** 작업자 수만큼 메모리가 뻥튀기됨. (OOM 위험 🚨)
  * **통신 비용:** 데이터를 포장(Pickling)하고 주고받는 시간이 든다.


* **교훈:** "1+1" 같은 잔심부름 시키면 택배비가 더 나온다. **무거운 작업(Heavy Task)**일 때만 써라.
