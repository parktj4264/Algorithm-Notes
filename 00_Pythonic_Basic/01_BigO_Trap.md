...
- **O(N)**: N이 2배면 시간도 대충 2배
    ...
    - 🚨 위험 구간: $N \approx 10,000,000 ~ (10^7)$ (1천만). 파이썬 순수 for문 기준 1천만 개부터 슬슬 1초를 넘기기 시작함. 1억 단위로 넘어가면 C 레벨로 구현된 numpy나 data.table 같은 벡터화 연산이 강제됨.
- **O(N^2)**: N이 2배면 시간은 대충 4배
    - $Cost(N) \approx N \times N$
    - 🚨 위험 구간: $N \approx 10,000 ~ (10^4)$ (1만). $N$이 1만만 되어도 연산량이 1억 번($10^8$)이라 1초 컷에 걸릴 위험이 큼. 
...
* **NumPy/R:** "결재"는 처음에 딱 한 번() 하고, 내부는 **C언어(기계어)**가 폭주 기관차처럼 달린다.
...
* **문제:** 리스트에서 데이터 찾으려면 방명록을 다 뒤져야 함 ().
* **해결:** **Key가 곧 주소(Address)다.**
* "찾기(Search)"를 **"수학 계산(Hash Function)"**으로 대체함.
* `Hash("Key") -> 메모리 582번지` (계산 끝, 바로 이동).


* **결과:** 데이터가 100만 개든 10억 개든, **단 한 번()의 접근**으로 데이터를 낚아챈다. (RAM을 가장 덜 괴롭히는 기술)
